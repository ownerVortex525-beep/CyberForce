...
function row(label, value, mono){ if (!value) return ''; return `<div class="rrow"><span class="rl">${label}:</span> <span class="rv ${mono ? 'mono' : ''}">${ml(value)}</span></div>`; }

function buildReportHTML(d, black){
  const ex = d.evidence.map((e, i) =>
    `<figure class="ex"><img src="${e.dataUrl}" width="${e.w}" height="${e.h}"><figcaption>Exhibit ${String.fromCharCode(65 + i)} - SHA-256: ${e.hash.slice(0, 16)}...</figcaption></figure>`
  ).join('');
  return `
  <div class="sheet ${black ? 'black' : ''}">
    ...
    ${d.suspect.photo ? `<img class="photo" src="${d.suspect.photo}" width="${d.suspect.photoW}" height="${d.suspect.photoH}" alt="Suspect photograph">` : ''}
